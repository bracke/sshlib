...
Implemented algorithm lists are advertised through `SSH_Lib.Algorithms`. Negotiation must select only implemented and advertised algorithms, preserve client preference order, reject unsupported intersections with `Unsupported_Feature`, select only implemented compression (`zlib@openssh.com`, `zlib`, or `none`), and place legacy SHA-1 `ssh-rsa` signing last as a bounded interoperability fallback.
