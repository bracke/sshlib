...
- `SSH_Lib.Forwarding.Start_Managed_Remote_Forward_Service` is the managed remote-forward boundary: it owns the `tcpip-forward` request, `forwarded-tcpip` accept loop, configured target connection, bounded byte pump, and cancel-on-exit path; applications can still choose lower-level request/cancel/accept primitives for custom policy.

## Host-key verification model

A valid host-key signature proves possession of the host private key for the exchange hash. A known_hosts match proves local trust policy. Both are required by default. `Verify_Known_Host` and `Strict_Host_Key` default to `True`, `Trust_On_First_Use` defaults to `False`, unknown hosts are rejected by default, and changed keys are rejected. Callers that intentionally accept a key can use `SSH_Lib.Known_Hosts.Append_Trusted_Host` or set `Trust_On_First_Use = True`; the TOFU path only handles unknown keys and still fails closed for mismatches, revocations, malformed records, and unsupported records.
...
### Weak-algorithm pruning and Terrapin strict key exchange

The default advertisement no longer offers the broken or deprecated legacy algorithms `hmac-md5`, `hmac-md5-96`, `hmac-md5-etm@openssh.com`, `hmac-md5-96-etm@openssh.com` (MD5 is broken), `3des-cbc`, or `diffie-hellman-group1-sha1` (1024-bit). These remain implemented and are still accepted if a user explicitly re-enables them through configuration; only the default KEXINIT proposal was narrowed, matching modern OpenSSH client defaults. SHA-1 MACs, AES-CBC ciphers, and `ssh-rsa` (SHA-1 host-key signatures) are retained as low-priority fallbacks.

The client implements the Terrapin countermeasure (CVE-2023-48795, strict key exchange). It advertises `kex-strict-c-v00@openssh.com`; when the server also advertises `kex-strict-s-v00@openssh.com`, both peers reset the packet sequence number to zero after NEWKEYS and reject any extraneous transport message (IGNORE/DEBUG/UNIMPLEMENTED/EXT_INFO) received during key exchange. This defeats the prefix-truncation attack against `chacha20-poly1305@openssh.com` and Encrypt-then-MAC ciphers: any packet an attacker injects into or deletes from the handshake produces a sequence mismatch on the first encrypted packet, which fails MAC verification. The markers themselves are covered by the exchange-hash signature, so a MITM cannot strip them without failing host-key verification. Strict key exchange is verified against OpenSSH 9.6+ (which logs `will use strict KEX ordering`).
...
Identity-file private material remains local secret input. The signing backend consumes validated Ed25519, ECDSA P-256/P-384/P-521, or RSA key material in memory, constructs a payload-bound signature blob, clears temporary seed/signature buffers where practical, and never logs private material or invokes external commands. P-384/P-521 ECDSA signing is implemented in `CryptoLib.ECDSA` and covered by generated-signature verification fixtures.
