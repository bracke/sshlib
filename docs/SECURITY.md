...
SSH_Lib does not use subprocess fallback for `ssh`, `git`, `ssh-agent`, or credential helpers. The exception is explicit `ProxyCommand`: when configured, `Sessions.Open` expands `%h`, `%p`, `%r`, and `%%` and carries SSH over that subprocess transport. `ProxyCommand none` is treated as disabled and is not executed. If a POSIX-style `sh` executable cannot be located, ProxyCommand fails closed with `Unsupported_Feature` rather than guessing a platform shell. Config loading and remote resolution still never shell out. ssh-agent support is through protocol-level communication only. Git helper functions quote repository paths before constructing remote command strings and reject NUL, CR, and LF.
