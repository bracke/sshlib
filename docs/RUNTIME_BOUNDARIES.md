...
The cipher package implements `aes256-ctr`, `aes192-ctr`, `aes128-ctr`, `aes256-cbc`, `aes192-cbc`, and `aes128-cbc` for live transport. Directional CTR and CBC cipher state is initialized from derived SSH keys and IVs during NEWKEYS fixture coverage; CBC chaining state is updated per protected packet. Unsupported cipher names continue to fail closed rather than being advertised or silently accepted.

### Ed25519 and RSA identity-file signing fixture path

The identity-file signing backend can produce payload-bound SSH signature blobs for parsed OpenSSH Ed25519 and RSA identities. RSA identity-file signing supports `rsa-sha2-512` and `rsa-sha2-256`, and live userauth falls back from SHA-512 to SHA-256 with legacy SHA-1 `ssh-rsa` only as a final interoperability fallback. Supported identity inputs include unencrypted OpenSSH RSA, OpenSSH Ed25519, PKCS#1 RSA PEM, PKCS#8 `BEGIN PRIVATE KEY` RSA, bcrypt-encrypted OpenSSH private keys, traditional AES/DES/3DES-CBC encrypted RSA PEM, and PBES2/PBKDF2 AES/3DES-CBC encrypted PKCS#8 RSA. Unsupported algorithms, unsupported encrypted envelopes, malformed keys, and wrong or missing passphrases continue to fail closed.
...
Unencrypted OpenSSH RSA, PKCS#1 RSA PEM, and PKCS#8 RSA private keys are parsed into the RSA SHA-2 signing path. OpenSSH bcrypt-encrypted private keys route through the Ada bcrypt_pbkdf derivation path and AES-CTR, AES-CBC, or 3DES-CBC private-section unwrap. Encrypted legacy PEM keys now support AES-128/192/256-CBC, DES-EDE3-CBC, and DES-CBC traditional PEM unwrap with the OpenSSL-compatible MD5 EVP_BytesToKey derivation, and encrypted PKCS#8 keys now support PBES2/PBKDF2-HMAC-SHA1, HMAC-SHA256, or HMAC-SHA512 with AES-128/192/256-CBC or des-EDE3-CBC before routing into the existing RSA DER parser. Unsupported ciphers, unsupported KDFs, malformed padding, and wrong passphrases still fail closed. The explicit non-retained passphrase option is present and passphrase bytes are not retained in session diagnostics. RSA public host-key verification for SHA-512 and SHA-256 continues to use the separate host-key verification path.
...
Ed25519 host-key verification and RSA SHA-512 verification are implemented. The runtime advertises `ssh-ed25519-cert-v01@openssh.com,rsa-sha2-512-cert-v01@openssh.com,rsa-sha2-256-cert-v01@openssh.com,ssh-rsa-cert-v01@openssh.com,ssh-ed25519,rsa-sha2-512,rsa-sha2-256,ssh-rsa` and supports legacy SHA-1 `ssh-rsa` signatures only as a last-resort fallback. Pass 70 adds positive RSA SHA-512 host-key verification coverage through `SSH_Lib.Crypto.Signatures.Verify`, plus signature/message tamper-negative checks.
...
Remaining explicit fail-closed boundaries are encrypted identity algorithms outside the implemented OpenSSH bcrypt AES-CTR/AES-CBC, legacy PEM AES-CBC/MD5, and PKCS#8 AES-CBC/PBKDF2-HMAC-SHA1/SHA256/SHA512 coverage, plus optional SSH algorithms that are not required for Version; RFC 8308 `ext-info-c` is advertised as an extension marker and skipped during KEX selection.
...
Pass 58 routes channel-owned stdin/stdout/EOF/close packet traffic through the retained session transcript. Remaining explicit fail-closed boundaries are encrypted identity algorithms outside the implemented OpenSSH bcrypt AES-CTR/AES-CBC, legacy PEM AES-CBC/MD5, and PKCS#8 AES-CBC/PBKDF2-HMAC-SHA1/SHA256/SHA512 coverage, plus optional SSH algorithms that are not required for Version; RFC 8308 `ext-info-c` is advertised as an extension marker and skipped during KEX selection.


## Phase 19 completeness pass 60 — PKCS#8 RSA identity files

PKCS#8 `BEGIN PRIVATE KEY` RSA identities are accepted by the identity-file parser. The parser validates the RFC 5208 `PrivateKeyInfo` sequence, requires the rsaEncryption object identifier, extracts the nested PKCS#1 `RSAPrivateKey` OCTET STRING, and then reuses the bounded PKCS#1 RSA parser added in pass 59. `BEGIN ENCRYPTED PRIVATE KEY` now unwraps bounded PBES2/PBKDF2 AES-CBC envelopes for HMAC-SHA1, HMAC-SHA256, and HMAC-SHA512 PRFs before feeding the same RSA DER parser; unsupported KDF/cipher combinations still return deterministic fail-closed statuses.
