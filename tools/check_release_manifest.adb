...
procedure Check_Release_Manifest is
   ...
   begin
      if not File_Contains (Path, Needle) then
         Fail ("missing manifest text in " & Path & ": " & Needle);
      end if;
   end Require_Text;
   ...
   begin
      if File_Contains (Path, Needle) then
         Fail ("stale manifest text in " & Path & ": " & Needle);
      end if;
   end Forbid_Text;
...
begin
   ...
   Require_Text ("CLAUDE.md", "Alire GNAT 15");
   Require_Text ("README.md", "Alire GNAT 15");
   Require_Text ("docs/TESTING.md", "Alire GNAT 15");
   Require_Text ("tools/check_release_toolchain.adb", "Require_Alire_GNAT_15");
   Require_Text ("tools/check_release_toolchain.adb", "gnat=15.2.1");
   ...
   if Failure_Count = 0 then
      Ada.Text_IO.Put_Line ("release manifest guard passed");
   else
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
   end if;
end Check_Release_Manifest;
